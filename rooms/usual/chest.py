...
def action(user, reply, text):
	...
	if text == actions[0]:
		...
		if random.random > 0.5:
			...
			if generator == 1:
				reply('Вы сундуке был Банан')
				user.add_item('loot', 'banana')
			elif generator == 2:
				reply('Вы сундуке был Крыло крысы-летяги')
				user.add_item('loot', 'bat_wing')
			elif generator == 3:
				reply('Вы сундуке был Указатель')
				user.add_item('neutral', 'sing')
			elif generator == 4:
				reply('Вы сундуке был Трезубец')
				user.add_item('special', 'trident', )
			else:
				reply('Вы сундуке был Амулет мага')
